...
const ListaExercicios = ({ whatsapp, aoFechar, API_URL, treinoExterno, modalidade }) => {
  ...
  const [grupoAtivo, setGrupoAtivo] = useState(() => {
    // Se vier um treino externo da IA, poderíamos parsear aqui. 
    // Por enquanto, carregamos a modalidade selecionada no App.js
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3">
                <input
                  type="number"
                  placeholder="kg"
                  className="bg-black/50 border border-white/10 rounded-2xl p-4 text-white font-black text-center w-24 outline-none"
                  value={cargas[idx] || ""}
                  onChange={(e) => setCargas({ ...cargas, [idx]: e.target.value })}
                />
                <button onClick={() => iniciarDescanso(60)} className="flex-1 bg-gray-800 p-4 rounded-2xl text-[10px] font-black uppercase">⏱️ Descanso</button>
              </div>
            </div>
          </div>
        ))}
